...
为前端生成该图像时：提供一个居中但极其宁静的表面，为 3-6 张 DOM 卡牌或少量可交互 3D 卡牌留出开阔的空间。不包含可读文本，不要出现具体的塔罗牌品牌，不要有任何可能喧宾夺主或与卡牌标签抢视线的花哨元素。
...
页面的推进来自牌与牌之间形成的关系，而不是翻开若干张牌后依次阅读。相同的牌放入不同阵位、与不同牌相邻时，应产生不同解释压力。
...
## 组合语法 (Combination Grammar)

- 至少两张牌必须通过相邻、对立、覆盖、朝向或阵位形成组合意义。
- 单张牌只提供象征碎片；解释文本应在组合成立后生成或改写。
- 用户移动一张已揭示的牌时，关系线、光线、声音与解释都应重新响应。
- Companion 不应是翻牌后弹出的卷轴；牌阵本身应沉静下来，成为可持续阅读的解释地图，计时与笔记栖息在阵位边缘。
- 禁止“翻三张牌 -> 三段独立介绍 -> 打开阅读面板”的固定循环。
...
### 3. 可选的三维牌桌 (Optional Three.js Card Table)

`oracle` 高度适合 `three-diegetic`，前提是抽取、翻牌、排列或组合卡牌直接推进解读。适合用 Three.js 表现牌堆厚度、翻面、落位、桌面光照和牌阵关系；牌面文字、解释卷轴与关键操作必须保留为清晰的 DOM。

- 推荐世界动作：从牌堆抽牌、拖入阵位、翻开牌面、旋转或对齐象征物。
- 牌进入阵位后，应同步改变关系连线、仪式光线、声音与解释文本。
- 为每张可交互牌提供 DOM 按钮或列表等价入口，使键盘用户可以抽取、翻开与放置。
- 移动端可以将自由拖拽降级为“选择牌 -> 选择阵位”，但不能丢失牌阵推进。
- 若只是三张卡牌翻面，优先使用 DOM/CSS 3D transform；不要为了材质炫光引入 Three.js。
- 禁止把牌桌做成竞技卡牌游戏、自由物理沙盒或让牌面正文成为 WebGL 纹理。
