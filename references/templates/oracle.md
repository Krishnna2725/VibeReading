...
- 拓扑：`combination`；意义必须由牌与牌、牌与阵位之间的关系产生。
- 默认渲染：`dom`；当抽取与摆放需要真实空间重量时可用 `three-diegetic`。
- 适配天气：薄雾、灰烬、花粉、稀疏风、风暴光；避免廉价星空粒子。
- 推荐阅读工具：`notes`、`stage-navigation`、`ambience-control`、`focus` 中选择 2-4 项。
- Companion 变形：牌阵降噪成为解释地图，阅读文字沿关系线或阵位边缘展开。
