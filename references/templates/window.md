...
### 3. 条件式三维景深 (Conditional Three.js Depth)

`window` 默认使用 `dom` 或 `canvas-enhanced`。只有观察距离、窗内外空间层次或天气体积本身承担阅读意义时，才考虑 `three-diegetic`。

- 适合的三维动作：推开窗扇、调整观察距离、转动风向标、将窗台物件移入不同光区。
- Three.js 更适合承担克制的景深、光照和窗外层次；天气与文字不应依赖 WebGL 才能访问。
- 窗扣、灯绳、天气切换和阅读纸条必须有 DOM 等价入口。
- 若只是视差、雨雪、雾气或昼夜变化，优先使用 CSS / Canvas。
- 禁止把窗外做成可自由飞行的 3D 风景，也禁止让技术展示破坏等待与凝视的节奏。
