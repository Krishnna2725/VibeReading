...
- 拓扑：`continuum` 或 `cyclical`；同一视野随等待、天气和光线持续变化。
- 默认渲染：`dom` / `canvas-enhanced`；只有观察距离本身有意义时使用 Three.js。
- 适配天气：雾、风、雨、雪、凝露；天气应成为观察对象。
- 推荐阅读工具：`ambience-control`、`focus`、`timer`、`notes` 中选择 2-4 项。
- Companion 变形：视野收束为窗边庇护所、纸条或田野笔记，不打开独立控制面板。
